...
public abstract class CrudService<D extends CrudDao<T>, T extends DataEntity<T>> extends BaseService {
	...
	/**
	 *	查询分页数据，扩展查询参数
	 * @param pageRequest	分页请求参数扩展类
	 * @author Yogaa
	 * @date 23:15 2023/12/15
	 */
	public Page<T> listPage(PageRequestExp pageRequest) {
		int total = dao.count(pageRequest);
		List<T> list = null;
        if(total > 0) {
            list = dao.listPage(pageRequest);
	...
	public List<T> listAll(PageRequestExp pageRequest) {
		return dao.listAll(pageRequest);
	}
	...
	public List<T> gets(String loginName) {
		return dao.gets(loginName);
	}

	public List<T>  getByrequires(String id) {
		return dao.getByrequires(id);
	}


	public List<T>  getBymilestone(String id) {
		return dao.getBymilestone(id);
	}

	public List<T>  getscheduleByid(String id) {
